...
<div class="container admin">
	<div class="row">
		<h1><strong>LISTER DES PATIENTS</strong>&nbsp;&nbsp;<a href="ajoutp.php" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-plus"></span>ajouter</a>&nbsp;&nbsp;<a href="con_docteur.php" class="btn btn-success btn-lg">Voir les docteurs</a></h1>
   ...
   require  'connex.php';
      $db = Database::db();
     $statement = $db->query('SELECT patients.id,patients.prenom,patients.nom,patients.symptone,secteur.nom_secteur FROM docteur INNER JOIN bloc on docteur.id = bloc.id_docteur INNER JOIN secteur on secteur.id=bloc.id_secteur INNER JOIN service on secteur.id=service.idsecteur iNNER JOIN secrecteur on secrecteur.id=service.idsecreteur INNER JOIN traitement on traitement.id_docteur=docteur.id INNER JOIN patients on patients.id=traitement.id_patient WHERE secrecteur.prenom="'.$_SESSION['sess_prenom'].'"');
     while($items = $statement->fetch())
     {
		echo '<tr>';
		echo '<td>'.$items['id'].'</td>';
		echo '<td>'.$items['prenom'].'</td>';	
		echo '<td>'.$items['nom'].'</td>';	
		echo '<td>'.$items['symptone'].'</td>';	
		echo '<td>'.$items['nom_secteur'].'</td>';
	
		echo '<td>
		<a href="con_add.php?id='.$items['id'].'" class="btn btn-primary
		"><span class="glyphicon glyphicon-eye-open"></span>Rendrez-vous</a><a href="con_view.php?id='.$items['id'].'" class="btn btn-default"><span class="glyphicon glyphicon-eye-open"></span>voir</a>
